...
if __name__ == "__main__":
    ...
    server.challenge = b'\xb6\xc5\x91e\xd2\x15|3\x94\x86\x84o\x1d\n\xde\t'
    server.send(server.challenge)
    client.generate_challenge()
    client_response = client.generate_client_response(server.challenge)
    client.send(client_response)
    challenge_response, new_challenge_hash, _username = client_response
    assert challenge_response == server.generate_challenge_response(
        new_challenge_hash
    )
    print("Client response:", challenge_response.hex(), new_challenge_hash.hex(), _username)
    ...
    # print(challenge_response, new_challenge_hash, _username)
    ...
    # print("Here's your flag", "FLag{123}")
