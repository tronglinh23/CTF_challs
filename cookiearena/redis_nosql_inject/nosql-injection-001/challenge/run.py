...
with app.app_context():
    db.create_all()
    usr = User(userid='admin', password=hashlib.md5(SECRET).hexdigest(), email=f'admin@cookie.io')
    db.session.add(usr)
    db.session.commit()
...
def signup():
    ...
    if request.method == "GET":
        ...
        userid = request.form.get("userid", "") #can control
        userpw = request.form.get("userpw", "") #can control
        ...
        try:
            usr = User(
                ...
                email=f"{userid}@cookie.io", # can be vulnerable
            )
            db.session.add(usr)
            db.session.commit()
        except:
