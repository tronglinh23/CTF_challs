...
if (isset($_POST['username']) && isset($_POST['password'])) {
  ...
  // username == admin to bypass first check, we cant use " or \ because it will be filtered
  $res = mysql_fquery($mysqli, 'SELECT * FROM users WHERE username = "%s"', [$username]);
  if (!mysqli_fetch_assoc($res)) {
     $message = "Username not found.";
     goto fail;
  }
  // need to bypass username and password
  ...
  // check 
  /*
  can phai truyen vao user name ket hop voi comment sau do su dung html special de xoa het cac phan tu 
  */
